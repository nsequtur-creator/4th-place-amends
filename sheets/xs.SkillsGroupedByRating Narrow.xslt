...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt">
  ...
  <xsl:template name="skills1">
    <xsl:variable name="skillcut" select="number(round((count(skills/skill[knowledge = 'False' and total &gt; 0]) + count(skills/skillgroup)) div 3))" />
  ...
  <xsl:template name="skills2">
    <xsl:variable name="skillcut" select="number(round((count(skills/skill[knowledge = 'False' and total &gt; 0]) + count(skills/skillgroup)) div 3))" />
	<xsl:variable name="skillthird" select="2*number(round((count(skills/skill[knowledge = 'False' and total &gt; 0]) + count(skills/skillgroup)) div 3))" />
    <xsl:variable name="sortedskills">
      <xsl:choose>
        <xsl:when test="$PrintSkillCategoryNames">
          <xsl:for-each select="skills/skill[knowledge = 'False' and total &gt; 0]">
            ...
            <xsl:sort select="rating" order="descending" />
            <xsl:sort select="name" />
            <xsl:if test="position() &lt;= $skillthird and false() = (position() &lt;= $skillcut)">
              <xsl:copy-of select="current()" />
            </xsl:if>
          </xsl:for-each>
        </xsl:when>
      ...
      <xsl:choose>
        <xsl:when test="$PrintSkillCategoryNames">
          <xsl:for-each select="skills/skill[knowledge = 'False' and total &gt; 0]">
            ...
            <xsl:sort select="rating" order="descending" />
            <xsl:sort select="name" />
            <xsl:if test="false() = (position() &lt;= $skillthird)">
              <xsl:copy-of select="current()" />
            </xsl:if>
          </xsl:for-each>
        </xsl:when>
  ...
  <xsl:template name="knowskills1">
    <xsl:variable name="skillcut" select="number(round((count(skills/skill[knowledge = 'True' and total &gt; 0])) div 3) - count(skills/skill[knowledge = 'True' and islanguage = 'True']))" />
	<xsl:variable name="skillthird" select="2*number(round((count(skills/skill[knowledge = 'True' and total &gt; 0])) div 3)- round(count(skills/skill[knowledge = 'True' and islanguage = 'True']) div 2))" />
    ...
    <xsl:variable name="sortedknowledgeskills">
      <xsl:choose>
        <xsl:when test="$PrintSkillCategoryNames">
          <xsl:for-each select="skills/skill[knowledge = 'True' and islanguage != 'True' and isnativelanguage != 'True' and rating &gt;= $MinimumRating]">
            <xsl:sort select="skillcategory" />
            <xsl:sort select="rating" order="descending" />
            <xsl:sort select="name" />
            <xsl:if test="position() &lt;= $skillcut">
        ...
        <xsl:otherwise>
          <xsl:for-each select="skills/skill[knowledge = 'True' and isnativelanguage != 'True' and rating &gt;= $MinimumRating]">
            <xsl:sort select="rating" order="descending" />
            <xsl:sort select="name" />
            <xsl:if test="position() &lt;= $skillcut">
              <xsl:copy-of select="current()" />
      ...
      <xsl:choose>
        <xsl:when test="$PrintSkillCategoryNames">
          <xsl:for-each select="skills/skill[knowledge = 'True' and islanguage != 'True' and isnativelanguage != 'True' and rating &gt;= $MinimumRating]">
            ...
            <xsl:sort select="rating" order="descending" />
            <xsl:sort select="name" />
            <xsl:if test="false() = (position() &lt;= $skillcut) and position() &lt;= $skillthird">
              <xsl:copy-of select="current()" />
            </xsl:if>
          </xsl:for-each>
        </xsl:when>
